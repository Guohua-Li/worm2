...
impl Default for Game {
    fn default() -> Self {
        Self {
            paused: false,
            worm:   Worm::default(),
            vocabulary:   consts::ZOO_ANIMALS.iter().map(|s| s.to_string()).collect(),
            word:  "rhinoceros".to_string(),
            foods: Vec::new(),
            game_state: consts::GameState::StartUI,
        }
    }
}
...
impl Game {
    fn create_foods(&mut self, canvas_size: Vec2) {
        ...
        while self.foods.len() < self.word.len() {
            let x = rng.gen_range(consts::DIAMETER..canvas_size.x-consts::DIAMETER);
            let y = rng.gen_range(consts::DIAMETER..canvas_size.y-consts::DIAMETER);
            let pos = Vec2 { x, y };
            let mut push = true;
            for fd in &self.foods {
                if (fd.position - pos).length() < 2.0 * consts::DIAMETER {
                    push = false;
                    break
                }
            }
    ...
    fn find_food(&mut self) -> Option<usize> {
        ...
        for i in 0..self.foods.len() {
            if  (self.foods[i].position-self.worm.head.position).length() < consts::DIAMETER {
                caught = Some(i);
                break;
            }
        }
    ...
    fn startup_ui(&mut self, ui: &mut Ui) {
        ...
        ui.horizontal(|ui| {
            if ui.add_sized([150., 50.], Button::new(RichText::new("  Start  ").size(20.0))).clicked() {
                self.game_state = consts::GameState::Init;
            }
            if ui.add_sized([150., 50.], Button::new(RichText::new("  Quit  ").size(20.0))).clicked() {
                self.game_state = consts::GameState::Exit;
            }
        });
    }
...
impl eframe::App for Game {
    ...
    fn update(&mut self, ctx: &Context, _frame: &mut eframe::Frame) {
        if self.game_state == consts::GameState::Exit {
        ...
        if key_down.contains(&Key::ArrowUp) {
            if self.worm.head.force < consts::MAX_FORCE {
                self.worm.head.force += consts::ACC_PER_FRAME;
            }
        } else if key_down.contains(&Key::ArrowLeft) {
            self.worm.head.inc_angle(-consts::ANG_PER_FRAME);
        ...
        CentralPanel::default().show(ctx, |ui| {
            if self.game_state == consts::GameState::StartUI {
                self.startup_ui(ui);
                return;
            }

            let canvas_size = ui.available_size();
            if self.game_state == consts::GameState::Init {
                self.create_foods(canvas_size);
                self.play_audio();
                self.game_state = consts::GameState::Play;
                return;
            } 

            if self.game_state == consts::GameState::Play && !self.paused {
                self.worm.move_me();
