...
impl Head {
    ...
    pub fn tick(&mut self) {
        ...
        vx += (self.angle.cos() * self.force - consts::FRICTION * vx) / consts::MASS;
        vy += (self.angle.sin() * self.force - consts::FRICTION * vy) / consts::MASS;
    ...
    pub fn paint(&self, painter: &Painter) {
        ...
        painter.circle( center, consts::RADIUS, Color32::DARK_GREEN, consts::WHITE_STROKE );
        let nose = Pos2 {
            x: self.position.x + consts::RADIUS * self.angle.cos(),
            y: self.position.y + consts::RADIUS * self.angle.sin()
        };
        painter.line_segment([center, nose], consts::WHITE_STROKE);
    }
}
